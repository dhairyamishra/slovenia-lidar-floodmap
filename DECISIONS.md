...
**Scope / caveat:** This is the riverine model applied to a coastal site — a **baseline**, not a sea-level-rise product. It flags low-flat coastal land (which partly overlaps SLR exposure) but cannot model tidal/marine inundation; that is the separate coastal "bathtub" mode (D19, pending). Cross-region top-20 entries are each normalised on their own per-region ruler, so they are "worst within region", not absolutely comparable (a known consequence of D17). **Forward-effect:** the no-data mask is now unconditional in the code, so a future *full* re-run would also turn inland no-data cells (under dense buildings, water bodies) transparent on the Ljubljana/Savinja PNGs. Only the 21 Koper tiles were processed this run, so the committed inland overlays are unchanged.
