...
- [x] Implement B0, B1, B2, M1, and M2.
- [x] Run spatially blocked cross-validation and altitude shortcut audit.
- [x] Test operating threshold/applicability using development targets; no candidate was selected because the gate failed.
- [ ] Calibrate probability only if enough independent labels/events exist.
- [x] Produce development model card with known failure modes.

**Exit gate: FAILED 2026-07-12 (D28).** The best development challenger, monotonic HGB with drainage features + TWI + slope and a 500 m channel-distance applicability mask, scores ROC-AUC/AP 0.7647/0.5819 versus mosaic HAND 0.7447/0.5049. It improves AP but misses the required +0.03 AUC gain (+0.0200) and 30% low-flat-control reduction (20.58%). Its Ljubljana score/elevation Pearson remains −0.6002 despite excluding absolute elevation. The locked test was not opened, and no replacement is approved.
...
| E008 | 2026-07-12 | Public comparison D30 | Frozen D19 0.925 review mask + committed 2 m DRSV Q100/validity grids | Exact cell categories inside official validity; no fitting | Derive official-only, D19-only, both, neither, missing-data, and unavailable classes; add click explanations and area shares | Ljubljana comparable area: 35.44% official-only, 5.05% D19-only, 4.30% both, 55.21% neither; 99.54% validity coverage | D19-only is calculated only inside validity with D19 data; outside validity is unavailable | Accept categorical display; supersede source-layer transparency blend without changing model-selection status |
| E009 | 2026-07-13 | Public coverage D31; comparison schema v2 | 391 CLSS tiles; refreshed DRSV validity/Q100; frozen evaluation contract unchanged | Public comparison expands; locked grids/splits do not | Add ten validity-selected 5x5 chunks, recalibrate regions, rerun all tiles, rasterize official geometry per current tile | Ljubljana comparison: 31.79% official-only, 11.59% D19-only, 7.53% both, 49.09% neither over 160.414 km2; fixed evaluation D19 AUC/AP 0.5690/0.3791 vs HAND 0.7049/0.5123 | Full-grid D19 Pearson/Spearman remains -0.4992/-0.4748; saturation gates fail | Accept coverage expansion only; keep D19 rejected and frozen, do not move the evaluation target |
